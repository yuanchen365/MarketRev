...
- 彙總：以月份為索引，對全部公司總和為 `total_revenue`。
...
3) 移動平均營收（Moving Average）
   - `avg_revenue_3m` = rolling_mean(total_revenue, 3)
   - `avg_revenue_6m` = rolling_mean(total_revenue, 6)
4) 累計（年初至今，YTD）與年增率
...
- 排除金融保險：比對 `industry` 欄位含「金融」或「保險」字樣的公司，移除後重新彙總一組 `total_revenue`，並套用與整體市場相同的指標流程。輸出檔名統一加上 `_ex_fin` 後綴。
...
- 市場層級 DataFrame：
  - `total_revenue`、`yoy_pct`、`yoy_pct_smooth_3m`
  - `index_2019_12_100`
  - `avg_revenue_3m`、`avg_revenue_6m`
  - `ytd_total`、`ytd_yoy_pct`、`ytd_yoy_pct_avg_3m`、`ytd_yoy_pct_avg_6m`
 
範例驗證（Validation）
- 使用 `scripts/verify_sample_metrics.py` 建立 2023-01 至 2024-03 的合成資料（科技、零售兩個產業，各佔 60%/40%），對 `aggregate_market_series()` 進行實算驗證。
...
維護與修改（Change Guide）
- 新增指標：在 `aggregate_market_series()` 中計算並命名欄位，於相應 `plot_*` 函式加入視覺化。
- 調整基期：在 `aggregate_market_series()` 內修改 `base_date` 邏輯（目前預設 2019-12，否則首期）。
- 產業圖選項：在 `plot_industry_facets()` 調整 `top_n` 或改為全產業 Facet。
- 排除金融保險：如需調整規則，更新 `scripts.build_market_growth._filter_out_finance()` 與 `ui/app.py` 中的 `split_ex_finance()`。
- 字型：如需指定字型，於 `plot_*` 函式內 `rcParams['font.family']` 調整。
